...
/// Agent Plan `GetAFPUsage` OpenAPI probe.
///
...
/// intentionally self-contained (no CodexBar imports) and offline: it produces
/// signature material for a request but performs no networking itself.
///
/// Spec assumptions (to be verified by Codex/Bee against the official Volcengine
/// signing reference before production use):
